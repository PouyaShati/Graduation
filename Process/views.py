...
from Process.forms import CreateProcessBlueprintForm, CreateQuestionSetForm
...
def create_process_blueprint(request):
    ...
    if request.method == 'POST':
        ...
        process_bp = Process_Blueprint(name=form['name']) # TODO add preprocesses
        process_bp.save()
...
def create_employee_task_blueprint(request):
    if request.method == 'POST':
        ...
        return HttpResponsleRedirect('/process/create_process_blueprint')
    else:
        return render(request, 'Process/create_process_blueprint.html', {'create_process_blueprint_form': CreateProcessBlueprintForm(label_suffix='')})
'''
